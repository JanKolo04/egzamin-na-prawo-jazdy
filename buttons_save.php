...
<body>
	...
	<?php
		...
		function save_current_question() {
			...
			$sql_check = "SELECT * FROM additional_data WHERE Id_user=$Id_user AND Category='$category' AND Poziom='{$_GET['poziom']}'";
			...
			if($query_check->num_rows == 0) {
				//save data
				$sql_insert = "INSERT INTO additional_data(Id_user, $zakres_struktury, Category, Poziom) VALUES($Id_user, $question, '$category','{$_GET['poziom']}')";
				$query_insert = mysqli_query($con, $sql_insert);
			}
			else {
				//update data
				$sql_update = "UPDATE additional_data SET $zakres_struktury=$question WHERE Id_user=$Id_user AND Category='$category' AND Poziom='{$_GET['poziom']}'";
